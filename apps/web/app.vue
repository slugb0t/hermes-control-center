...
<template>
  <div class="min-h-screen md:grid md:grid-cols-[17rem_minmax(0,1fr)] max-md:pb-[calc(4.8rem+env(safe-area-inset-bottom))]">
    <aside
    ...
    <main class="min-w-0 p-5 max-md:px-4 max-md:pb-4 max-md:pt-[max(1rem,env(safe-area-inset-top))]">
      <NuxtPage />
    </main>

    <nav
      class="fixed inset-x-3 bottom-[calc(0.75rem+env(safe-area-inset-bottom))] z-20 hidden grid-cols-5 gap-1 rounded-[1.4rem] border border-white/10 bg-zinc-950/90 p-2 shadow-[0_18px_80px_rgba(0,0,0,0.5)] backdrop-blur-2xl max-md:grid"
